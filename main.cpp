...
//questo serve per il font con cui voglio fare scritte, è necessario?, è giusto?
  ...
  std::vector<sf::Text> xTicks; // creazione degli oggetti di testo per le tacchette sugli assi
  ...
  for (float x=0.f; x<= maxXValue; x += 2.f) { 
      ...
      tickName.setString(std::to_string(static_cast<int>(x))); //qui viene impostato il contenuto del testo dell'oggetto tickName CON IL VALORE numerico DI X.std::to_string converte il valore numerico in una stringa. static_cast<int> ci assicura che il valore di y venga convertito in un intero prima di essere convertito in stringa.
      tickName.setPosition(graphPosition.x + (x / maxXValue) * graphSize.x - 10, graphPosition.y + graphSize.y + 10 ); //valori a caso
      xTicks.push_back(tickName); // l'oggeto tickName viene aggiunto al vettore xTicks che conterrà tutte le etichette delle tacchette lungo l'asse delle x
  }
 for (float y=0.f; y<= maxXValue; y += 20.f) { 
      ...
      tickName.setCharacterSize(14);
      tickName.setFillColor(sf::Color::Black);
      tickName.setString(std::to_string(static_cast<int>(y))); //qui viene impostato il contenuto del testo dell'oggetto tickName CON IL VALORE numerico DI Y.std::to_string converte il valore numerico in una stringa. static_cast<int> ci assicura che il valore di y venga convertito in un intero prima di essere convertito in stringa.
      tickName.setPosition(graphPosition.x - 30, graphPosition.y + graphSize.y - (y / maxYValue) * graphSize.y -7 ); //valori a caso
...
for (const sf::Text& tick : xTicks) { window.draw(tick); } //ciclo range-based . Serve per disegnare l'oggetto tick (corrente) sulla finestra. Tutte le etichette vengono disegnate una dopo l'altra seguendo l'ordine in cui sono nel vettore xTicks.
for (const sf::Text& tick : yTicks) { window.draw(tick); }
  window.display();
  }
  }
